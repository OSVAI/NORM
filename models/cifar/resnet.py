...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10, marginal_relu=False):
    ...
    def forward(self, x, is_feat=False, preact=False):
        ...
        if self.marginal_relu: # Marginal relu
            if isinstance(self.layer1[0], Bottleneck):
                bn4 = self.layer3[-1].bn3
            elif isinstance(self.layer1[0], BasicBlock):
                bn4 = self.layer3[-1].bn2
            margin4 = self.get_margin_from_BN(bn4)
            margin4 = margin4.unsqueeze(1).unsqueeze(2).unsqueeze(0).detach()
            f3 = torch.max(x, margin4)
        else:
            f3 = x
